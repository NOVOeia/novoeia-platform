...
export default function AdminEmailTemplates() {
  ...
  const selectedMeta = useMemo(
    () => templates.find(item => item.id === selectedId) || EMAIL_TEMPLATE_CATALOG.find(item => item.id === selectedId),
  ...
  async function saveAll() {
    try {
      ...
      await platformApi.saveEmailTemplates(payload);
  ...
  async function sendTest() {
    try {
      setBusy(true);
      await platformApi.saveEmailTemplates(Object.fromEntries(
        templates.map(item => [item.id, {
          subject: item.subject,
          html: item.html,
          enabled: item.enabled !== false,
    ...
    <div className="novo-page">
      <div className="novo-page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p>Edita los correos automáticos del sistema. Usa variables como <code>{'{{fullName}}'}</code>.</p>
        </div>
