...
export default function FunnelAdminPanel({
  embedded = false,
  open = true,
  ...
  const [internalSettings, setInternalSettings] = useState(() => cloneFunnelSettings(initialSettings));
  ...
  useEffect(() => {
    if (embedded) return;
    if (!open) return;
    setInternalSettings(cloneFunnelSettings(initialSettings));
    setNotice({ text: '', type: '' });
    setActiveTab('hero');
  }, [embedded, open, initialSettings]);
  ...
  const shellClass = embedded ? 'fam-embedded' : 'fam-overlay';
  const panelClass = embedded ? 'fam-panel fam-panel-embedded' : 'fam-modal';
  ...
  return (
    ...
    >
      ...
      <section
        ...
        aria-label="Administración del funnel"
      >
        ...
        <header className="fam-header">
          <div className="fam-header-left">
            ...
            <div>
              <h2>{embedded ? 'Configuración del funnel' : 'Administrar página de ventas'}</h2>
        ...
        <footer className="fam-footer">
          ...
          <div className="fam-footer-actions">
            ...
            <button type="button" className="fam-btn-primary" disabled={saving || busy} onClick={saveSettings}>
              <Save size={15} />
              {saving || busy ? 'Guardando...' : embedded ? 'Guardar configuración del funnel' : 'Guardar cambios'}
            </button>
          </div>
        </footer>
      </section>
...
const CSS = `
  ...
  .fam-panel-embedded {
    ...
    border-radius: 14px;
    box-shadow: none;
    background: var(--novo-card, #f8fafc);
    border: 1px solid var(--novo-border, rgba(148,163,184,.15));
  }
